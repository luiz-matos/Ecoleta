...
Cadastro de pontos de coleta de resíduos, com API em Node, Express e SQLite e front-end em React com TypeScript e mapa do Leaflet.

Fiz em 2020 durante a Next Level Week 1 da Rocketseat. Em 2026 voltei a ele: atualizei as dependências, que não instalavam mais, troquei o Create React App pelo Vite, corrigi os bugs, adicionei o upload da imagem do ponto e organizei o código. A pasta do app mobile, que nunca passou do template do Expo, saiu do repositório.
...
- Tela de confirmação depois do cadastro, voltando para a home em 2 segundos
- Itens de coleta selecionáveis pelo teclado (Tab, Espaço e Enter)
- Layout do formulário adaptado a telas pequenas
...
└── src/
    ├── server.ts                   Express, arquivos estáticos e middleware de erro
...
└── src/
    ├── routes.tsx                  / (home) e /create-point (cadastro)
    ...
    ├── components/MapController/   clique no mapa e recentralização
    └── pages/                      Home e CreatePoint
```
...
- **Mapa.** O `MapContainer` do react-leaflet só usa `center` e `zoom` na montagem. O `MapController` recentraliza o mapa quando a geolocalização responde e repassa o clique que marca o endereço.
...
- **Mesmo resultado.** Conferi a organização com testes fora deste repositório: um roteiro com 25 verificações da API e 8 cenários do formulário (Vitest e Testing Library). As saídas foram idênticas antes e depois.
